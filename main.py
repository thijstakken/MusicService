...
ydl_opts = {
    ...
    'outtmpl': '/usr/app/src/music/%(playlist)s/%(title)s.%(ext)s',# save music to the /music folder. and it's corrosponding folder which will be named after the playlist name
    ...
    'download_archive': '/usr/app/src/database/downloaded.txt',    # this will update the .txt file which serves as a database for which files have already been downloaded
...
# reads and saves playlist URL's in a list
    with open('/usr/app/src/database/playlists.txt') as file:
